...
const BATCH_SIZE = 1000; // setting pool size to 1 to test connections are properly released
...
const allValues = [];
beforeAll(async () => {
  await db.query(__1.sql`CREATE SCHEMA streaming_test`);
  await db.query(__1.sql`CREATE TABLE streaming_test.values (id BIGINT NOT NULL PRIMARY KEY);`);
...
function addTests(name, withConnection) {
  test(`${name} - node streaming`, async () => {
    ...
    const results = await withConnection(async db => {
      const results = await new Promise((resolve, reject) => {
        const results = [];
        db.queryNodeStream(__1.sql`SELECT * FROM streaming_test.values`, {
          highWaterMark: 100
  ...
  test(`${name} - await streaming`, async () => {
    ...
    const results = await withConnection(async db => {
      ...
      for await (const {
        id
      } of db.queryStream(__1.sql`SELECT * FROM streaming_test.values`, {
        batchSize: 100
      })) {
        results.push(id);
      }
