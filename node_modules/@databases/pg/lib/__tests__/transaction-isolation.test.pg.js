...
async function transaction(db, {
  ...
  const [{
    sum
  }] = await db.query(__1.sql`SELECT SUM(value) as sum FROM mytab WHERE class = ${from};`);
  await wait();
  await db.query(__1.sql`INSERT INTO mytab (class, value) VALUES (${to}, ${sum})`);
}

async function prepare() {
  await db.query(__1.sql`CREATE SCHEMA IF NOT EXISTS transaction_isolation_test`);
  await db.query(__1.sql`DROP TABLE IF EXISTS mytab;`);
  await db.query(__1.sql`CREATE TABLE mytab (
...
test('default isolation level', async () => {
  ...
  await Promise.all([db.tx(async tx => await transaction(tx, {
    from: 1,
    to: 2,
    wait: waitA
  })), db.tx(async tx => await transaction(tx, {
    from: 2,
    to: 1,
    wait: waitB
  }))]);
...
test('serializable', async () => {
  await prepare();
  const [waitA, waitB] = createSynchronizer();
  await expect(Promise.all([db.tx(async tx => await transaction(tx, {
    from: 1,
    to: 2,
    wait: waitA
  }), {
...
test('serializable - with retry', async () => {
  ...
  const [waitA, waitB] = createSynchronizer();
  let attempts = 0;
  await Promise.all([db.tx(async tx => {
    attempts++;
