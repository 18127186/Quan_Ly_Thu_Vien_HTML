...
function quoteString(str, quoteChar) {
  ...
  for (const c of str) {
    if (c === quoteChar) escaped += c + c;else escaped += c;
  }
...
function assertAscii(str, db, unicodeAvailable) {
  if (!/^[\u0001-\u007f]+$/.test(str)) {
    throw new Error(`${db} identifiers may only contain ASCII characters${unicodeAvailable ? ` (to use unicode, pass {extended: true} when escaping the idnetifier)` : ``}`);
  }
}
...
function assertUnicode(str, db) {
  ...
  // U+0080 .. U+FFFF
  if (!/^[\u0001-\uffff]+$/.test(str)) {
    throw new Error(`${db} identifiers should onoy contain charcters in the range: U+0001 .. U+FFFF`);
  }
}
